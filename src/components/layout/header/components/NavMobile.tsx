...
export const NavMobile = ({ routes }: { routes: Array<{ title: string; path: string; Icon: ElementType }> }) => {
    ...
    return (
        <ul className="flex lg:hidden items-center bg-gray-900/70 border border-white/10 rounded-full px-4 py-2 backdrop-blur-md">
                        ...
                        <Link
                            ...
                            className={`relative flex items-center justify-center w-9 h-9 rounded-full transition-all duration-200 ${
                                isActive
                                    ...
                                    : "text-gray-500 hover:text-orange-300 hover:bg-orange-500/10"
                            }`}
                        >
                            <Icon className="w-4 h-4" />
                            {/* Active dot */}
                            {isActive && (
                                <span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-[3px] h-[3px] rounded-full bg-orange-300" />
                            )}
                        </Link>
